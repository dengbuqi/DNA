...
经过 17 个版本、70+ 次迭代的系统性实验后，**DNA 在 MNIST 上成功达到 91%+** ——批量训练和在线学习均达成目标，且 EML 算子细胞（exp(w₁·x) − ln(w₂·x)）作为 sigmoid 的替代方案成功验证。
...
| **v17** | **EML 细胞 (exp(w₁·x) − ln(w₂·x))** | **90.53%** 🆕 | **EML 算子替代 sigmoid 验证成功** |
...
```python
output = sigmoid(gain * (w · x + bias))

  输入 (784 个像素)
    └── 每个输出类分配 N 个细胞 (8~64/类)
...
```python
output = sigmoid(exp(w₁ · x) − ln(w₂ · x + bias))

  输入 (784 个像素)
    └── 每个细胞有两套权重 w₁, w₂
        ├── w₁: exp 分支，控制"兴奋"幅度
        └── w₂: ln 分支 + bias，控制"抑制"基线

学习规则 (纯局部，无 BP):
  Δw₁    = lr × error × sig' × exp(w₁·x) × x
  Δw₂    = lr × error × sig' × (−1/(w₂·x)) × x
  Δbias  = lr × error × sig' × (−1/(w₂·x))
```
...
| `w1_scale` | 0.1 | EML exp 分支 lr 缩放 |
| `w2_scale` | 3.0 | EML ln 分支 lr 缩放 |

---

## 项目历程

这个项目起源于对"可动态增删细胞的神经网络"的好奇。经历 70+ 次实验后，最重要的教训是：
